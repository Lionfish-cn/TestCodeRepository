...
import com.code.repository.jpa.lettercode.LetterCodeBlackListRepository;
...
public class VerifyCodeController {
    ...
    public void sendShortLetterCode(HttpServletRequest request, HttpServletResponse response) {
        ...
        Example example = Example.of(VerifyCodeBlackList.builder().phone(phone).build());
        ...
        if(!optionals.isPresent()){//如果手机号不存在黑名单中，才允许发送验证码
            ...
            if (mn > 0) {
                ...
                VerifyCodeBlackList blackList = VerifyCodeBlackList.builder().happenTime(new Date()).phone(phone).reason(message).build();
